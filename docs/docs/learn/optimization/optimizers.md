...
9. [**`TextualFrequencyOptimizer`**](../../api/optimizers/TextualFrequencyOptimizer.md): A zero-shot optimizer that generates semantically equivalent paraphrases of each predictor's instruction and selects the candidate with the highest geometric-mean unigram frequency (via [`wordfreq`](https://pypi.org/project/wordfreq/)). Because it does not use task labels, it requires no training data. Task performance should still be measured with a DSPy metric after optimizing. Requires `pip install wordfreq`.
